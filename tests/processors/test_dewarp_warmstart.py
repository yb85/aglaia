...
basin. Only the global shape transfers (the cubic slopes, pvec[6:8]);
...
def _d():
    return PageDewarper(DewarpOption(backend="powell"))
...
def test_remember_then_seed_same_side():
    ...
    d._remember_warm_curl(solved, _buf("left"))

    cold = np.zeros(20, dtype=np.float32)
    d._seed_warm_curl(cold, _buf("left"))
...
def test_median_of_recent_ignores_a_single_runaway():
    ...
    for cubic in ([0.10, 0.20], [0.10, 0.20], [5.0, 8.0]):
        p = np.zeros(20, dtype=np.float32)
        p[6:8] = cubic
        d._remember_warm_curl(p, _buf("left"))
    cold = np.zeros(20, dtype=np.float32)
    d._seed_warm_curl(cold, _buf("left"))
...
def test_other_side_not_seeded():
    ...
    d._remember_warm_curl(solved, _buf("left"))
    cold = np.zeros(20, dtype=np.float32)
    d._seed_warm_curl(cold, _buf("right"))           # different side
...
def test_non_finite_curl_is_ignored():
    ...
    d._remember_warm_curl(bad, _buf("left"))
    cold = np.zeros(20, dtype=np.float32)
    d._seed_warm_curl(cold, _buf("left"))

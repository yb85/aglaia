...
See `lib/workers/cli.py` for the full argument schema.
"""
...
# imports `jax`. See lib/processors/PageDewarper.py for the leak this prevents.
...
def _run_headless(cfg: CliConfig) -> int:
    from lib.workers.headless import run
    return run(cfg)
...
def _qt_app() -> "QApplication":
    ...
    try:
        ...
        from lib.i18n import install_translator
        try:
    ...
    try:
        from lib.gui.theme import apply_modern_theme
        from lib.app_data import db as cfg
        try:
            with cfg.session() as conn:
    ...
    try:
        from lib.gui.plugin_trust import prompt_pending_plugins
...
def _choice_from_cfg(cfg: CliConfig) -> Optional["StartupChoice"]:
    ...
    from lib.gui.StartupWindow import StartupChoice, StartupWindow

    if cfg.source == "project":
        from lib.storage import slug_from_project_file
...
def _bootstrap_with_choice(app, choice, cfg: CliConfig) -> int:
    ...
    from lib.gui.StartupWindow import StartupWindow
    from lib.workers.Calibrator import load_calibration
    from lib.workers.ImportHelpers import (
        catchup_active_scans, enqueue_image_files, enqueue_pdf_files,
        reprocess_active_scans,
    )
    from lib.workers.Initializer import (
        create_processing_chain, initialize, load_pipeline_def,
    )
    from lib.workers.PDFprocessor import create_pdf_from_images
    from lib.storage.db import open_db
    from lib.storage.repo import PipelineRepo, ProjectRepo, ScanRepo

    # ── splash ────────────────────────────────────────────────────
    # Palette-aware bg + text so the loading panel reads correctly on
    # both dark and light themes (was hardcoded dark slate).
    from lib.gui.colors import active_palette_name as _palette_name
    ...
    try:
        from lib.app_data import db as app_db
    ...
    from lib.gui.PageWarningDialog import maybe_show_heuristic_warning
...
def _load_active_pipeline_yaml(project_dir: Path, slug: str) -> str:
    from lib.storage import resolve_existing_project_db
    db_path = resolve_existing_project_db(project_dir, slug)
    if db_path is None:
        return ""
    import sqlite3
...
def main(argv: list[str] | None = None) -> int:
    ...
    from lib.workers.cli import run_list_commands
    ...
    while True:
        if first and cfg.has_inputs():
            ...
            if app.property("aglaia_slim_before_reopen"):
                ...
                try:
                    from lib.workers.slim_export import slim_in_place
...
if __name__ == "__main__":
    # MUST be the first call inside `__main__` for PyInstaller-frozen
    # builds: without `freeze_support`, every `multiprocessing` worker
    # re-runs this script as a fresh process — i.e. each spawned chain
    # worker pops its own Qt app + StartupWindow, leading to the
    # fork-bomb-style "multiple Aglaïa instances" symptom on .app
    # launch. The call is a no-op when running from source.

...
```mermaid
flowchart TD
    Init["Initializer.initialize(mode)<br/>YAML → args + args.options<br/>(lib/workers/Initializer.py)"]
...
### Initializer (`lib/workers/Initializer.py`)
...
- Option dataclasses for each YAML `options:` block come from the processor registry (`lib/processors/registry.py:option_classes()`) — auto-discovered, not a hand-maintained map.

### IntegratedProcessingChain (`lib/workers/IntegratedProcessingChain.py`)
...
- Persistence is **in-worker**: each worker opens the project SQLite DB and writes every stage's image blob + `nodes` row through a `Persister` (`lib/storage/persister.py`). There is no separate writer process and no loose files on disk — everything lives in the `.agl` DB.
...
In the GUI, `ProcessMonitor` (`lib/workers/ProcessMonitor.py`) drains the queue and re-emits them as Qt signals on the main thread.
...
- **Imports (GUI import panel / `--headless`)**: `lib/workers/ImportHelpers.py` (`enqueue_pdf_files` / `enqueue_image_files`) extracts or renders pages via `PDFprocessor`, wraps each in an `ImageBuffer`, and feeds the same chain.
...
flowchart TD
    ...
    Main["main / log_queue drain (lib/workers/headless.py)"]

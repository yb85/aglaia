...
from lib.processors import utils
...
class DewarpOption(AbstractProcessorOption):
    ...
    #                   field. See lib/processors/sheet_models.py.
...
class PageDewarper(AbstractImageProcessor):
    ...
    def inject_step_options(cls, step_opts: dict, args) -> dict:
        ...
        from lib.workers.Initializer import _is_real_calibration  # local import
    ...
    def __init__(self, options: DewarpOption):
        ...
        from lib.processors.sheet_models import MODEL_FLAT_SPLINE
        ...
        if requested == "powell" or not HAS_JAX:
            ...
            if try_mlx:
                try:
                    from lib.processors.page_dewarp_mlx import (
            ...
            if active != "mlx":
                if pad_allowed:
                    try:
                        from lib.processors.page_dewarp_padded import (
    ...
    def _set_backend_sheet_model(self) -> None:
        ...
        try:
            if self.backend == "mlx":
                from lib.processors.page_dewarp_mlx import set_sheet_model as _s
                _s(self.sheet_model, self.spline_modes, self.twist)
            elif self.backend == "jax":
                from lib.processors.page_dewarp_padded import set_sheet_model as _s
                _s(self.sheet_model, self.spline_modes, self.twist)
        except Exception:
            pass
    ...
    def process(self, img_buf: ImageBuffer) -> ImageBuffer:
        ...
        if success and not coward_skip:
            ...
            from lib.processors import sheet_models

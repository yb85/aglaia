...
Adding a new processor:
  1. Drop `lib/processors/<NewProc>.py` containing a subclass of
...
from lib.processors.abstraction import AbstractImageProcessor, AbstractProcessorOption
...
def _synthesize_option_class(name: str, options: dict[str, Any]) -> type:
    ...
    cls.__module__ = "lib.processors.registry"
    return cls
...
def _discover_once() -> None:
    ...
    for _, mod_name, _ in pkgutil.iter_modules(_pkg.__path__):
        if mod_name.startswith("_") or mod_name in {"abstraction", "registry",
            ...
            continue
        try:
            module = importlib.import_module(f"lib.processors.{mod_name}")
        except Exception as e:
            print(f"[registry] failed to import lib.processors.{mod_name}: {e}")
            continue

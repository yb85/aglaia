...
from lib.processors.layout_backends.base import LayoutBackend
...
def probe_active_backend(name: str = "auto") -> str:
    ...
    if is_mac:
        try:
            from lib.processors.layout_backends import apple_vision as _av
            if _av.AppleVisionBackend.is_available():  # type: ignore[attr-defined]
                return "apple_vision"
        except Exception:
            try:
                from lib.processors.layout_backends.apple_vision import AppleVisionBackend
                AppleVisionBackend()
                return "apple_vision"
            except Exception:
                pass
    try:
        from lib.processors.layout_backends.east import _resolve_model_path as _east_path
        _east_path()
        return "east"
    except Exception:
        pass
    try:
        from lib.processors.layout_backends.dbnet import _resolve_model_path as _db_path
        _db_path()
        return "dbnet"
    except Exception:
        pass
...
def get_backend(name: str = "auto") -> LayoutBackend:
    ...
    name = (name or "auto").lower()
    if name == "dbnet":
        from lib.processors.layout_backends.dbnet import DbnetBackend
        return DbnetBackend()
    if name == "east":
        from lib.processors.layout_backends.east import EastBackend
        return EastBackend()
    ...
    if name == "auto":
        ...
        if is_mac:
            loaders.append(lambda: __import__(
                "lib.processors.layout_backends.apple_vision",
        ...
        loaders.append(lambda: __import__(
            "lib.processors.layout_backends.east",
            fromlist=["EastBackend"]).EastBackend())
        loaders.append(lambda: __import__(
            "lib.processors.layout_backends.dbnet",

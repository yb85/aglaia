...
# these open as a project. Source of truth is lib.storage.
...
def run_list_commands(cfg: "CliConfig") -> bool:
    ...
    if cfg.list_pipelines:
        from lib.app_data import pipelines_dir
        print("Pipelines:")
    ...
    if cfg.list_ocr:
        print("OCR engines:")
        try:
            import lib.workers.ocr  # noqa: F401 — side-effect registers engines
...
def resolve_pipeline_path(pipeline_arg: Optional[str]) -> Optional[Path]:
    ...
    candidate = repo_root / "config" / "pipelines" / f"{pipeline_arg}.yaml"
...
def default_parent_dir(cfg: CliConfig) -> Path:
    ...
    from lib.app_data import default_documents_dir

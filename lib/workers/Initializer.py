...
def probe_capabilities() -> list[tuple[str, bool, str]]:
    ...
    if _probe_import("paddleocr") and _probe_import("mlx_vlm"):
        try:
            from lib.workers.ocr.paddle_vl import _paddle_weights_dir
    ...
    if _probe_import("surya"):
        try:
            from lib.workers.ocr.surya import _resolve_llama_server
    ...
    # backend resolver (lib/processors/layout_backends/dbnet.py).
    found, src = _probe_model_file(
        "AGLAIA_DBNET_MODEL",
...
def print_startup_info(workers: int | None = None) -> None:
    ...
    try:
        from lib.app_data import app_data_dir, cache_dir, log_dir
    ...
    try:
        from lib.app_data import plugins as _plugins
...
def _option_map() -> dict:
    """Lazy registry view — adding a processor only needs its file in `lib/processors/`."""

...
from lib.workers.Initializer import create_processing_chain, initialize, load_pipeline_def
from lib.workers.ImportHelpers import (
    catchup_active_scans, enqueue_image_files, enqueue_pdf_files,
)
from lib.workers.PDFprocessor import create_pdf_from_db
from lib.workers.md_export import write_markdown
...
from lib.storage.repo import PipelineRepo, ProjectRepo, ScanRepo, OcrRepo
...
def run(cfg: CliConfig) -> int:
    ...
    if pipeline_path is None:
        pipeline_path = Path("config/pipelines/book_curved_x2.yaml").resolve()

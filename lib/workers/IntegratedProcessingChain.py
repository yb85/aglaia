...
class IntegratedProcessingChain:
    ...
    def _rss_sampler_loop(self):
        ...
        from lib.workers.macos_mem import phys_footprint_mb, thread_count
    ...
    def _watchdog_loop(self):
        ...
        from lib.workers.macos_mem import phys_footprint_mb
    ...
    def _worker_loop(elements_config: List[SimpleChainElement],
        ...
        from lib.workers.macos_mem import phys_footprint_mb as _phys_footprint_for_pid
        ...
        def run_pipeline(buf: ImageBuffer, start_idx: int):
            ...
            for i in range(start_idx, N):
                ...
                if log_queue is not None:
                    ...
                    # logs the same shape (see lib/workers/oplog.py).
                    if not _os.environ.get("AGLAIA_QUIET_STAGE"):
                        from lib.workers.oplog import format_op, fmt_size_chain
            ...
            if current.parent_node_id is not None and current.scan_id is not None:
                ...
                replay_ok = False
                try:
                    from lib.workers.Replay import replay_branch

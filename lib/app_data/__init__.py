...
def plugins_dir(kind: str | None = None) -> Path:
    ...
    Created on access. See `lib/app_data/plugins.py` for the trust gate
...
def _models_dir_override() -> Path | None:
    ...
    config DB to keep `lib.app_data` import-free at process start."""
    try:
        from lib.app_data import db as _db  # noqa: WPS433

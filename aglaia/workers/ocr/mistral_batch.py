...
def submit(api_key: str, img_rows: list[dict], run_ids: list[int],
    ...
    for ci, (pdf_bytes, n) in enumerate(chunks):
        ...
        # Always request header/footer extraction (free extra fields); the
        # markdown-export toggle decides whether to wrap or inline them.
...
def page_to_result(page: dict, page_w: int, page_h: int,
    ...
    Mistral page object — the RAW markdown for md_export plus the rich page
    (``meta.mistral_page``, header/footer included) so deferred batch results

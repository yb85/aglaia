...
from aglaia.storage.repo import ScanRepo
...
def enqueue_pdf_files(*, db_path: str, pipeline_version_id: int,
                      ...
                      progress_cb: Optional[Callable[[int, int], None]] = None,
                      log_queue=None):
    from aglaia.workers.pdf_extract import open_pdf
    ...
    for pdf, n_pages in zip(pdfs, page_counts):
        if n_pages == 0:
            continue
        from aglaia.workers.pdf_extract import render_page
        doc = open_pdf(pdf)
        try:
...
def reprocess_active_scans(*, db_path: str, pipeline_version_id: int,
    ...
    """
    from aglaia.storage.repo import NodeRepo, ImageRepo, BranchRepo
    conn = open_db(db_path)
    try:
        conn.commit()
...
def reprocess_branch(*, db_path: str, pipeline_version_id: int, chain,
    ...
    """
    from aglaia.storage.repo import NodeRepo, ImageRepo
...
def scans_needing_catchup(db_path: str) -> list[int]:
    ...
    from aglaia.storage.db import open_db as _open
    conn = _open(db_path)
    try:
        rows = conn.execute(

...
Currently exposes:
  ...
  * `Card` — rounded QFrame with `objectName == "Card"` so the QSS in
    `aglaia.gui.theme` styles it as an elevated container.
"""
...
class Switch(QAbstractButton):
    ...
    exposed as Qt properties so the QSS in `aglaia.gui.theme` can theme them
...
class Card(QFrame):
    """Rounded container styled via QSS in `aglaia.gui.theme` (selector

...
Each "card" is `aglaia.gui.widgets.Card` styled via the QSS in
`aglaia.gui.theme`. Booleans render as the `Switch` toggle pill.
...
from aglaia.processors.option_specs import COMMON_OPTION_SPECS
...
from aglaia.gui.widgets import Card, Switch
...
from aglaia.app_data import pipelines_dir as _pipelines_dir
...
class _StepDelegate(QStyledItemDelegate):
    ...
    def paint(self, p: QPainter, opt, idx):  # noqa: N802
        ...
        from aglaia.gui.colors import COLOR_FONT_PRIMARY
...
class PipelineEditorWidget(QWidget):
    ...
    def _build(self):
        ...
        self._preview = None
        if self._db_path:
            from aglaia.gui.PipelinePreviewPanel import PipelinePreviewPanel
